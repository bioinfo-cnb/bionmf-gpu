 ...
 ***********************************************************************/
/**********************************************************
 * native_file_converter.c
 *	Program to perform "native"-binary -- text file conversions.
 *
 * WARNING:
...
struct input_arguments {
	...
	bool is_nat_bin;		// Input file is ("native") binary.
	bool numeric_hdrs;		// Has numeric columns headers.
	bool numeric_lbls;		// Has numeric row labels.
	bool save_non_native;		// Save matrix in a non-"native"-binary file.
};
 ...
 */
static void print_file_converter_help( char const *restrict execname, FILE *restrict file )
{
	...
	fprintf( file, "\n\t<< Binary -- ASCII-text file converter >>\n\n"
		"Usage:\n\t%s <filename> [ -bcrs ]\n\t%s [ -h | --help ]\n\n", execname, execname );

	fprintf( file, "<filename>\tInput data matrix (mandatory if 'help' is not requested).\n\n");

	fprintf( file, "-B,-b\tInput file is in \"native\" binary format (i.e., with the compiled types for matrix data and dimensions).\n"
	...
	fprintf( file, "-S,-s\tSaves input matrix as (non-\"native\") binary format (i.e., double-precision data and unsigned int's; "
			"disabled by default).\n\n");
					...
					struct input_arguments *restrict arguments )
{
	...
	bool l_is_nat_bin = false;	// File is (non-"native") binary.
	bool l_numeric_hdrs = false;	// Has numeric columns headers.
	bool l_numeric_lbls = false;	// Has numeric row labels.
	bool l_save_non_native = false;	// Save matrix in a "native"-binary file.
	 ...
	 *	-s (save matrix in a non-"native"-binary file)
	 */
	...
	while ( (opt = getopt_long( argc, (char *const *restrict) argv, ":BbCcHhRrSs", longopt, NULL) ) != -1 ) {
		...
		switch( opt ) {
			...
			// Save input matrix in a non-"native"-binary file.
			case 'S':
			case 's':
	...
	l_arguments.is_nat_bin = l_is_nat_bin;
...
int main( int argc, char *restrict const *restrict argv )
{
	...
	bool const is_nat_bin = arguments.is_nat_bin;			// File is "native" binary.
	...
	bool const save_non_native = arguments.save_non_native;		// Save input matrix in a non-"native"-binary file
	...
	if ( is_nat_bin ) {	// Input file is "native" binary

		printf("\nFile selected as \"native\" binary (i.e., with the compiled types for matrix data and dimensions). Loading...\n");
	...
	// Input file is "native" binary. Saves as text.
	else if ( is_nat_bin ) {
	...
	// Input file is text. Saves as "native" binary.
	else {
