...
#			It is equivalent to: 'all multi_gpu'.
#
...
#
#	clobber:	Removes code generated by the 'all' target.
#			It is equivalent to 'clobber_single_gpu clobber_tools'.
#
#	clobber_all:	Removes the entire binary directory.
#
#	clobber_single_gpu, clobber_multi_gpu, clobber_tools:
#			Removes the specified executable and object files.
#
#	clean:		Removes object files generated by the 'all' target.
#			It is equivalent to 'clean_single_gpu clean_tools'.
#
...
#			precision arithmetic, or are not supported.
#
...
#			Ignored on files not compiled with NVCC.
#
...
#			Ignored on newer GPU architectures.
#
#	PTXAS_FLAGS:	Additional flags for PTX code compilation, which
#			generates the actual GPU assembler.
#
#	MPICC_CPPFLAGS, MPICC_CFLAGS, MPICC_INCLUDES, MPICC_LDFLAGS, MPICC_LDLIBS:
...
ifeq ($(SM_VERSIONS),)
	...
	#	  Capability 1.2 or lower. The compilation process may fail if any
	#	  of them is specified below and the parameter "SINGLE" is set to '0'.
	#	* Compute Capability 1.0 is DEPRECATED on CUDA Toolkit version 6.0 or
	#	  greater.
	#
	SM_VERSIONS := 13 20 30 35
...
##
 ...
 #	can be found in the 'doc' folder of your CUDA Toolkit, or at URL:
...
cuda_nvcc_INCLUDES  := $(NVCC_INCLUDES) $(INCLUDES) --include-path $(incdir)
...
c_INCLUDES := $(INCLUDES) -I$(incdir)

# Additional flags for CUDA host code compiled with CC
cuda_cc_INCLUDES := -I$(nvcc_incdir)
...
single_gpu_INCLUDES	:= $(tools_INCLUDES) -I$(nvcc_incdir)
...
# Rule to compile default programs
.PHONY: all All ALL
all All ALL : single_gpu tools


# Rule to compile all programs.
.PHONY: all_programs All_Programs ALL_PROGRAMS
all_programs All_Programs ALL_PROGRAMS : all multi_gpu
...
check_cuda_path :
ifeq ($(CUDA_HOME),)
	@echo -e $(cuda_home_not_found_message) >&2
	@exit 1
endif
...
check_sm_versions :
ifneq ($(SM_VERSION),)
	@echo -e $(unknown_sm_version_message) >&2
	@exit 1
endif
ifneq ($(SINGLE),1)
ifneq ($(strip $(unsupported_sm_versions)),)
	@echo -e $(unsupported_sm_versions_message) >&2
endif
endif
...
clobber Clobber CLOBBER : clobber_tools clobber_single_gpu


.PHONY: clobber_all Clobber_All CLOBBER_ALL
clobber_all Clobber_All CLOBBER_ALL :
...
clean Clean CLEAN : clean_tools clean_single_gpu


.PHONY: clean_all Clean_All CLEAN_ALL
clean_all Clean_All CLEAN_ALL :
...
help_sm_versions_message := "\n\
 ...
 found in the 'doc' folder of your CUDA Toolkit, or at URL\n\
 ...
 Warning:\n\
	\t* Double-precision floating-point data are NOT supported on Compute\n\
	\t  Capability 1.2 or lower. The compilation process may fail if any\n\
	\t  of them is specified and the parameter "SINGLE" is set to '0'.\n\
	\t* Compute Capability 1.0 is DEPRECATED on CUDA Toolkit version 6.0 or\n\
	\t  greater.\n\
