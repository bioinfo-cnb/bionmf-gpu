...
fast_opt_level	   := fast
# Note: change fast_opt_level to $(opt_level) if your $(CC) compiler does not recognize the option '-Ofast'.
common_fast_CFLAGS := -march=native -ffast-math -fbranch-target-load-optimize -fomit-frame-pointer \
			-ftree-loop-distribution -funroll-loops -mfpmath=sse -msse4.1 -funsafe-loop-optimizations

# ALERT: TODO: (1) Linux- and MacOsX-specific flags. (2) Fix the word size (-m32, -m64).
...
#	Otherwise, CC is ignored and the default C/C++ compiler is invoked ('gcc' for UNIX, and 'cl.exe' for Windows).
...
not_in_opencc := -Wextra -Wstrict-overflow% -W%type-limits -ftree-loop-distribution -mfpmath=% -msse4.% -W%unsuffixed-float-constants \
		-march=% -f%branch-target-load-optimize -f%branch-target-load-optimize2 -Ofast% -fpch% -W%pch -f%pch \
		-f%unsafe-loop-optimizations

export OPENCC_FLAGS := $(filter-out $(not_in_opencc),$(common_CFLAGS)) -W $(OPENCC_FLAGS)
opencc_fast_CFLAGS  := $(filter-out $(not_in_opencc),$(c_only_fast_CFLAGS)) -ffast-stdlib -ffast-math -gcc -inline -gnu4
